...
| 层级 | 平台 | 选型理由 |
|------|------|----------|
| 前端托管 | Vercel | 专为前端优化、全球CDN加速、与GitHub深度集成、自动HTTPS |
| 后端服务 | Render | 支持Node.js运行时、自动扩展、免费套餐可用 |
| 数据库 | Railway | 托管PostgreSQL服务、自动备份、便捷管理界面 |
| 代码托管 | GitHub | 版本控制、CI/CD触发、团队协作 |
...
- **前端 (Vercel)**: 推送到 `main` 分支自动触发构建部署
- **后端 (Render)**: 推送到 `main` 分支自动触发构建部署
...
|------|------|----------|
| 1.0 | 2024-12-19 | 初始版本 |
| 1.1 | 2024-12-28 | 添加生产环境部署架构（Vercel + Render + Railway） |
